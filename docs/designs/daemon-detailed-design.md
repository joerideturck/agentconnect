...
- Env/secrets changes affect host spawn. Reconcile replaces the old host once its running turns settle (section 5.2), so the next start uses the new values.
...
apply(plan):
  ...
  - runtime change           -> applies to next new session; do not interrupt active
  - workspace change         -> cut active turns, re-materialize the checkout
  - cron change              -> Scheduler upsert/remove
```

A **runtime change** is one in the reconciler's host-spawn signature (runtime, sandbox, model, effort, modes, memory backend, env, secrets, skills, description). When the agent has no turn in flight its processes stop at once and the next turn starts a fresh one. Otherwise each process with running turns keeps serving them and stops once they settle; idle processes stop at once. A turn that would start on a still-serving old process waits until it stops, then runs on its successor — with one shared process that is every new turn of the agent, with a process per session only that session's follow-ups. The wait is bounded by `limits.configRespawnDrainMs` (5 min on every daemon: no service manager is waiting on it). A turn still running at the limit, or one still starting up when the change lands, is cut with its durable inbox row kept and replayed on the new process, and a cut turn's conversation is told so; a webchat turn has no row and is told to send its message again. A **workspace change** still cuts active turns at once (terminal, with the same notice), since the checkout itself is re-pointed.
